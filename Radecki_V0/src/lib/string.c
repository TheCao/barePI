...
void StringPutChar (TString *pThis, char chChar, size_t_r nCount /* = 1 */);
void StringPutString (TString *pThis, const char *pString);
void StringReserveSpace (TString *pThis, size_t_r nSpace);
...
size_t_r StringGetLength (TString *pThis)
{
	assert (pThis != 0);
...
void StringPutChar (TString *pThis, char chChar, size_t_r nCount)
{
	assert (pThis != 0);
...
void StringReserveSpace (TString *pThis, size_t_r nSpace)
{
	...
	size_t_r nOffset = pThis->m_pInPtr - pThis->m_pBuffer;
	size_t_r nNewSize = nOffset + nSpace + 1;
